...
function rawValuesCsv(values: MonsterValue[]): string {
  const rows = [
    ['id', 'name', 'level', 'hp', 'race', 'size', 'element', 'ev_per_kill', 'spawn_weighted_ev', 'best_map', 'best_map_spawns', 'top_drops'],
    ...values.map((value) => [
      value.monster.id,
      value.monster.name,
      value.monster.level,
...
export default function App() {
  ...
  return (
    <div className="app-shell">
      ...
      <section className="panel filters-panel" aria-label="Farming filters and pricing assumptions">
        <div className="control-group">
          <div className="control-heading"><span>Search and ranking</span><small>Start broad, then narrow by map, monster traits, or value.</small></div>
          <div className="primary-controls">
            ...
            <label><span>Sort</span><select value={sortBy} onChange={(event) => setSortBy(event.target.value as SortKey)}><option value="expectedValue">EV / kill</option><option value="mapScore">Spawn-weighted EV</option><option value="spawns">Best-map spawns</option><option value="level">Level</option><option value="hp">HP</option><option value="name">Monster name</option></select></label>
        ...
        <div className="control-group">
          ...
          <div className="quick-filters">
            ...
            <label><span>Min EV / kill</span><input inputMode="numeric" placeholder="0" value={minEv} onChange={(event) => setMinEv(event.target.value)} /></label>
          </div>
        </div>
        ...
        <details className="advanced-controls"><summary>Advanced filters and assumptions</summary><div className="range-grid"><RangeControl label="Level" min={levelBounds.min} max={levelBounds.max} low={levelMin === '' ? levelBounds.min : parseNumber(levelMin)} high={levelMax === '' ? levelBounds.max : parseNumber(levelMax)} onLowChange={(value) => setLevelMin(String(value))} onHighChange={(value) => setLevelMax(String(value))} /><RangeControl label="Best-map spawns" min={spawnBounds.min} max={spawnBounds.max} low={spawnMin === '' ? spawnBounds.min : parseNumber(spawnMin)} high={spawnMax === '' ? spawnBounds.max : parseNumber(spawnMax)} onLowChange={(value) => setSpawnMin(String(value))} onHighChange={(value) => setSpawnMax(String(value))} /></div><div className="advanced-grid"><label><span>Drop multiplier</span><input type="number" min="0" step="0.5" value={settings.dropMultiplier} onChange={(event) => updateSetting('dropMultiplier', parseNumber(event.target.value) as Settings['dropMultiplier'])} /><small>Multiplies normal drop chance and caps at 100%.</small></label><label><span>Overcharge rate</span><input type="number" min="1" step="0.01" value={settings.overchargeRate} onChange={(event) => updateSetting('overchargeRate', parseNumber(event.target.value, 1) as Settings['overchargeRate'])} /></label><label><span>Poring Coin price</span><input type="number" min="0" step="500" value={settings.poringCoinPrice} onChange={(event) => updateSetting('poringCoinPrice', parseNumber(event.target.value) as Settings['poringCoinPrice'])} /></label><button type="button" className="secondary-button" onClick={resetSettings}>Reset assumptions</button></div></details>
      </section>

      <section className="metrics-grid"><Metric label="Matching mobs" value={filtered.length.toLocaleString()} /><Metric label="Highest EV" value={compactZeny(metrics.highestEv)} /><Metric label="Median EV" value={compactZeny(metrics.medianEv)} /><Metric label="Highest spawn-weighted EV" value={compactZeny(metrics.highestMapScore)} /><Metric label="Price profile" value={settings.uaro ? 'UARO' : 'NPC'} /><Metric label="Manual prices" value={manualPriceCount.toLocaleString()} /></section>
      <FormulaPanel />
...
function IntroPanel({ onDismiss }: { onDismiss: () => void }) {
  return <section className="panel intro-panel"><div><p className="eyebrow">How to use this tool</p><h2>Find a farming target, then tune the assumptions.</h2><p>Search by monster, item, ID, or map. Adjust UARO, Overcharge, drop multiplier, and manual prices. Select a mob to inspect drops and enter your kill rate for a zeny/hour estimate.</p></div><button type="button" className="secondary-button" onClick={onDismiss}>Dismiss</button></section>;
}
...
function Help({ tab }: { tab: TabKey }) {
  const text = {
    farms: 'Best farms is the main mob table. EV is recalculated from current pricing assumptions. Spawn-weighted EV is EV multiplied by best-map spawns.',
    ...
    raw: 'Raw data is an audit view of the filtered browser dataset. Export CSV or JSON when you want to inspect results elsewhere.',
  }[tab];
...
function MonsterResults({ dataStatus, values, selectedId, activeFilterCount, clearFilters, onSelect }: { dataStatus: DataStatus; values: MonsterValue[]; selectedId: number | null; activeFilterCount: number; clearFilters: () => void; onSelect: (id: number) => void }) {
  if (dataStatus === 'loading') return <EmptyState title="Loading generated monster data" body="Preparing the generated Hercules monster, item, and spawn dataset." />;
